...
async function startServer() {
  ...
  // Bulletproof header interceptor to guarantee 1-year browser cache for static files
  app.use((req, res, next) => {
    ...
    res.writeHead = function (this: any, statusCode: number, ...args: any[]) {
      ...
      if (headers && typeof headers === "object") {
        ...
        // If writeHead was called without a headers object, we define it on-the-fly if needed
        if (isStaticAsset) {
